...
const createRecipeObject = function (data) {
  const { recipe } = data.data; //reformatting the object
  return {
    id: recipe.id,
    title: recipe.title,
    ...
    //the key is only returned if the key exists
...
// for development to clear bookmarks when you need to comment out init on line 110
  ...
  try {
    const ingredients = Object.entries(newRecipe)
      ...
      .map(ing => {
        ...
        if (ingArr.length !== 3)
          throw new Error(
            'Wrong ingredient format! Please use the correct format! (⊙_⊙;)'
          );
